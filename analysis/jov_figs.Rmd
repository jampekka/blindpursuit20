...
The object was tracked closely during occlusion, the average total positional error (angular displacement between gaze positionand target object position) being 5.91 degrees at target reappearance (SD 4.92). Correspondingly, the average absolute phase differencewas 0.07 turns *(SD 0.07)* and radius difference 2.01 degrees (SD 1.87). 
...
In the discrimination task, the success rate by participant rangedfrom 32 to 75 percent, with an overall rate of *57 percent*. When gaze was close to target (position error at most 1.5 degrees), the overallsuccess rate was over 90 percent
...
When the object was visible,  itwas tracked by smooth pursuit movements with some small-amplitude saccades (frequency *1.49* saccades/s, amplitude median *1.83 degrees*).  When the object was occluded, the trajectory was tracked mostly by saccadic eye movements - more saccades were made(*2.88* saccades/s), with a median amplitude of *4.49 degrees*
...
The absolute magnitude of phase difference was greater for landing than for launch points for all but oneparticipant (binomial test, p = .021; 
...
overall medians of absolute phase difference were 0.03 turns and *0.05* turns for launch and landingpoints, respectively).In contrast, during visible periods, even though not all saccades were catch-up saccades, they were very smallin terms of the phase covered and did not show clear anticipatory nature (medians of absolute phase difference were 0.01 turns forboth launch and landing points).  

As can be seen in Figure 4, the phase lead of saccade landing points during occlusion was manifoldcompared to landing points in visible periods (0.05 vs. 0.01 turns, respectively).
...
During periods when the tracked object was visible, the total phase angle coveredwas almost entirely by smooth pursuit (median 91 percent of total phase covered).  During periods of occlusion, considerably less ofthe phase angle was covered by smooth pursuit (median 36 percent; see Figure 5).
...
In occluded periods, pursuit duration was shorter (see *Appendix X*) and the phase angle covered by pursuit segments was smaller com-pared to visible periods
...
pg_lmm1 <- lmerTest::lmer(phasedist ~ seg_dur*target_class2 + (seg_dur*target_class2|participant),
...
slopes <- broom::tidy(pg_lmm1, effects = "fixed") %>%
...
Regression slopes were comparedto the constant speed of the target to assess pursuit gain, which was as high as *0.98* during visible periods but only *0.33* during occlusion. 

*Based on random effects of the model, pursuit gain was at least 0.95 in visible and between 0.16 and 0.49 in the occluded condition for all participants*
...
```
```{r}
ggsave(filename="jov_figures/pursuit_saccade_gain.pdf", plot=p, width=19, height = 9, units = "cm")
...
diff1 <- ts_interp %>%
  ...
  mutate(GM = rollmean(gt_abs_dist, k=15, fill=NA, align="center")) %>% 
  ungroup %>%
  filter(!is.na(GM)) %>%
...
diff2 <- ts_interp_phase %>%
  ...
  group_by(participant) %>%
  arrange(participant, occl_ts2) %>%
  mutate(GM = rollmean(phase_diff_unwrap, k=15, fill=NA, align="center")) %>%
  ungroup %>%
  filter(!is.na(GM)) %>%
  ggplot(aes(occl_ts2, GM)) +  
    geom_vline(xintercept = 0, alpha = .3, linetype = "dashed")  +  
  ...
  scale_color_d3() + 
  scale_y_continuous(labels = formatterphase, breaks = seq(-.75, .25, by = .25)*2*pi, limits = c(-2.4, .5)) +
  scale_x_continuous(limits = c(-.5, 3)) +
  jov_theme
...
diff3 <- ts_interp_radius %>%
  ...
  group_by(participant) %>%
  arrange(participant, occl_ts2) %>%
  mutate(GM = rollmean(radius_diff, k=15, fill=NA, align="center")) %>%
  ungroup %>%
  filter(!is.na(GM)) %>%
    ggplot(aes(occl_ts2, GM)) +  
    geom_vline(xintercept = 0, alpha = .3, linetype = "dashed")  +  
...
*Calculated by taking the participant-wise median of linearly interpolated time series. Lines smoothed by rolling mean (window width 0.24 s).*



## Phase and radius SD over time

the phase difference showed clear increase in variance over time:  after theinitial 0.5 seconds, the phase difference SD increased at about 0.025 turns per second (LMM of phase difference SD predicted by time,with random intercept and slope by participant; p ¡ .001, *t(9) = 6.018* by Satterthwaite’s degree of freedom method). *Based on the random effects of the model, the slopes for participants varied between 0.01 and 0.04 turns per second.* 
