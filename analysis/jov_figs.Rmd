...
isochronic_lines <- df_trials %>%
   filter(participant == sbj, trial_number == trialn) %>%
    ...
    filter(round(trial_ts,6) %in% round(isochronic_ts[c(1:8, 10, 12, 14:19)],6)) %>%
...
df_trials %>%
    ...
    geom_path(aes(gazeX_s, gazeY_s, color = seg_fill, group=1), 
              alpha = 1, inherit.aes = F, size = .8, linejoin = "bevel", lineend = "round") +
...
plot_segm_xy <- function(sbj, trialn) {
  xcoord <- df_trials %>%
    ...
    geom_path(aes(trial_ts, target_x, color = target_class, group=target_class), 
              alpha = 1, size = 1) +
    geom_path(aes(trial_ts, gazeX_s, color = seg_fill, group=1), size = .8, alpha = 1,
    ...
    geom_path(aes(trial_ts, target_y, color = target_class, group=target_class), 
              alpha = 1, size = 1) +
    geom_path(aes(trial_ts, gazeY_s, color = seg_fill, group=1), size = .8, alpha = 1,
              inherit.aes = F, linejoin = "bevel", lineend = "round") +
...
trial_xy_times <- plot_segm_xy(7, 371)
...
plot_segm_phaserad <- function(sbj, trialn) {
  phase <- df_trials %>%
    ...
    geom_path(aes(trial_ts, unwrap_target_phase_R, color = target_class, group=target_class), 
              alpha = 1, size = 1) +
    geom_path(aes(trial_ts, unwrap_gaze_phase_R, color = seg_fill, group=1), size = .8, alpha = 1,
              inherit.aes = F, linejoin = "bevel", lineend = "round") +
    ...
    geom_path(aes(trial_ts, target_radius, color = target_class, group=target_class), 
              alpha = 1, size = 1) +
    geom_path(aes(trial_ts, gaze_radius, color = seg_fill, group=1), size = .8, alpha = 1,
              inherit.aes = F, linejoin = "bevel", lineend = "round") +
...
trial_times <- plot_segm_phaserad(7, 371)
...
phase_covered <- ggarrange(plotlist = phaseplots, common.legend = T, ncol = 2, legend = "right", align = "hv",
                           widths = c(2.18,3), font.label = list(size = 12, hjust = .5)) 
...
```{r}
p <- ggarrange(pursuitplot, NULL, ampref_trial, ncol = 3, widths = c(1, 0.05, 1), 
...
pretest_pos <- dfall %>%
  ...
  group_by(participant, trial_number) %>%
  filter(row_number() == max(row_number())) %>%
  ggplot(aes(target_x, target_y, colour = factor(success_trial) %>% fct_rev)) + 
...
(phase_rates <- target_phases %>%
  ...
  jov_theme +
  facet_rep_wrap(~participant, labeller = label_both, ncol = 5) + 
    coord_fixed())
...
(pursuit_dur <- seg_data_uncut$pursuit_timeseries %>%
  ...
  mutate(target_class2 = case_when(
   ...
   mutate(target_class = str_to_title(target_class)) %>%
   filter(target_class2 != "Visible and occluded") %>%
  ggplot(aes(seg_dur, fill = target_class %>% fct_rev)) + geom_density(alpha = .65) +
  facet_wrap(~participant, labeller = label_both, ncol = 5) +
...
seg_data_uncut$pursuit_timeseries %>%
  filter(trial_type == "occluded_trial", target_class %in% c("visible", "occluded")) %>%
  mutate(target_class2 = case_when(
    target_class == "visible" & !both_class ~ "Visible only",
    target_class == "visible" & both_class ~ "Visible and occluded",
   ...
   mutate(target_class = str_to_title(target_class)) %>%
   filter(target_class2 != "Visible and occluded")  %>%
  group_by(target_class2, participant) %>%
  summarise(md = median(seg_dur, na.rm=T)) %>%
  arrange(target_class2, md)
...
pmd <- seg_data_uncut$sacc_timeseries %>%
  filter(trial_type == "occluded_trial", target_class %in% c("visible", "occluded"), !first_seg, !last_seg) %>%
  group_by(target_class, participant, trial_number) %>%
  summarise(median_amp = median(dist, na.rm=T)) %>%
  mutate(target_class = str_to_title(target_class)) %>%
  ungroup
  

(part_ampfreq <- df_trials %>% 
  filter(trial_type == "occluded_trial", target_class %in% c("Visible", "Occluded")) %>%
  group_by(participant, trial_number, target_class) %>%
  summarise(trial_dur = max(trial_ts) - min(trial_ts), 
            scount = n_distinct(saccade_index, na.rm=T)) %>%
  mutate(trial_freq = scount/trial_dur) %>%
  full_join(pmd) %>%
  filter(trial_freq < 7.5, median_amp < 10) %>%
  gather(var, value, trial_freq, median_amp) %>%
  mutate(var = recode(var, trial_freq = "Frequency\n(saccades/s)", median_amp = "Median amplitude\n(degrees)")) %>%
  ggplot(aes(participant, value, fill = target_class %>% fct_rev)) + geom_boxplot(alpha = .65) +
  lemon::facet_rep_wrap(var~., scales = "free_y", ncol = 1, switch = "y") +
...
```{r}
ggsave(filename="jov_figures/saccade_distr.pdf", plot=part_ampfreq, width=18, height = 11, units = "cm")
```
...
radts <- saccades %>% 
  ...
  lemon::facet_rep_wrap(~participant, labeller = label_both, ncol = 5) +
  jov_theme
