...
# 
# df_trials %>%
#     filter(participant == sbj, trial_number == trialn, ts <= isochronic_ts[3]) %>% tail(110) %>% head(1) %>% pull(ts)

# can't see number 3 and 9 -> adjust. make sure this is visible!!
isochronic_ts[3] <- isochronic_ts[3] - 0.4613226


df_trials %>%
    mutate(across(c(target_x, target_y, gazeX_s, gazeY_s), .names = "{col}_segm")) %>%
    filter(round(ts,3) %in% round(isochronic_ts[1:8],3)) %>%
