...
# add segment_fill to colour data points by segment
# add is_gap to denote gaps between (circular) blocks; TODO: fix this later
# TODO: check that participant grouping works
# df <- df %>%
...
#   mutate(seg_fill = begin_seg_class) %>%
#   fill(seg_fill) %>%
#   mutate(seg_fill = if_else(is_gap, NA_character_, seg_fill)) %>%
#   fill(seg_fill, .direction = "up") %>%
#   ungroup 

#  fill(seg_fill_end, .direction = "up") %>%
  # remove points where the two filled segments do not match (-> discrepancies caused by gaps) BUT this also removes all points where segment changes; therefore shift begin by 1
#  mutate(seg_fill = if_else(seg_fill_begin == seg_fill_end, seg_fill_begin, NA_character_)) %>%
...
**TODO**: remove segments over "gaps" between blocks

```{r fig.width=10}
# collect begin and end points to new df(s)
# segments <- df %>%
# #  filter(!is.na(timept), is_cue == 1) %>%
...
segments <- df %>%
 ...
 # select(participant, ts, begin_segment_index:end_seg_class) %>%
...
df_trials %>%
  ...
  ungroup %>%
  mutate(success_trial = ifelse(success_trial == 1, "correct", "false")) %>%
  ggplot(aes(phase_diff, fill = trial_type)) + 
  ...
  labs(title = "Phase difference between gaze and target (at target reappearance)",
       subtitle = "Participant-wise distributions, visible/occluded trials") +
  theme_bw() 
