...
TODO: To degrees + in the same subplot?
Longer period
...
df %>%
    filter(participant == sbj, between(ts, ts_center - 20, ts_center + 20), 
           !is.na(trial_number)) %>%
#  gather(var, value, head_screen_x, head_deg_x) %>%
  ggplot(aes(ts, head_deg_x)) + geom_line(alpha = .6) +
  scale_y_continuous(name = "Degrees",
...
#    facet_grid(var~., switch = "y", scales = "free_y") +
    labs(x = "time (s)", title = title, y = "", color = "") +
...
#    mutate(var = str_remove(var, "head_deg_")) %>%
 ...
 #   facet_grid(var~., switch = "y", scales = "free_y") +
  ...
  scale_y_continuous(limits = c(-20, 20), name = "Y (degrees)",
    ...
    p <- ggarrange(xplot, yplot, ncol = 1)
...
pdf("headtracking.pdf", width = 12, height = 7)
...
## 1. Muutos sakkadiseksi hidden vs. visible
Mitta: sakkadifrekvenssi (saccades / second)
...
broom.mixed::augment(fit) %>%
  gather(var, value, freq, .fitted) %>%
  ggplot(aes(var, value, fill = target_class)) + 
  geom_boxplot(alpha=.3) + 
  scale_y_continuous(limits = c(0,10)) +
  theme_bw() +
  labs(title = "Observed values vs. predictions based on the fixed effect of target class")

coefs <- broom.mixed::tidy(fixef(fit)) %>% spread(names, x)


trial_saccades %>%
  mutate(predicted_freq = if_else(target_class == "visible", coefs$`(Intercept)`,
                                  coefs$`(Intercept)` + coefs$target_classhidden)) %>%
  ggplot(aes(factor(participant), freq, fill = target_class, color = target_class)) + 
  geom_boxplot(alpha = .4) +
  geom_line(aes(participant, predicted_freq), inherit.aes = F) +
...
rects <- df %>% 
  filter(participant == "7", between(trial_number, 369, 371), 
         target_class %in% c("visible", "hidden")) %>%
  group_by(trial_number, target_class) %>%
...
p7_launch <- saccades %>% 
  filter(participant == "7", between(trial_number, 369, 371), timept == "launch") %>%
...
p7 <- df %>% 
  ...
  geom_text(data = rects, aes(xmin + .5, -17, label = round(freq,1)), inherit.aes = F) +
...
Nice to have:
Myös frekvenssin muutos ajan yli okkluusiossa
Ikkuna tai kumulatiivinen (skaalattuna trialien määrällä)
...
saccadetimes <- saccades %>%
  ...
  select(participant, seg_index, occl_ts, trial_number) %>%
  mutate(participant = factor(participant)) %>%
...
occl_ts_saccades %>%
  ...
  ungroup %>%
  ggplot(aes(occl_ts, fct_reorder(trialn, max_ts) %>% fct_rev(), group = trialn)) + geom_line(alpha = .4) +
...
occl_ts_saccades %>%
  ...
  mutate(cumulative_saccades = cumsum(!is.na(seg_index)),
         saccade_freq = cumulative_saccades/occl_ts) %>%
  ggplot(aes(occl_ts, saccade_freq, group = trial_number)) + geom_line(alpha = .4) +
  scale_y_continuous(limits = c(0,20)) +
  facet_wrap(~participant)

# need to scale by duration!!
occl_ts_saccades %>%
  arrange(participant, occl_ts) %>%
  ...
  summarise(saccades = sum(!is.na(seg_index)),
            ...
            occl_bin_bound = min(occl_ts)) %>%
  group_by(participant) %>%
  mutate(saccades_per_trial = saccades/trials,
         cumulative_saccades = cumsum(saccades),
         cs_per_trial = cumsum(saccades_per_trial)) %>%
  ungroup %>%
  ggplot(aes(occl_bin_bound, cs_per_trial, color = participant)) + 
  geom_line() +
  facet_wrap(~participant)
...
occl_ts_saccades %>%
  ...
  mutate(saccades_per_trial = saccades/trials,
         cumulative_saccades = cumsum(saccades),
         saccade_freq = cumulative_saccades/occl_bin_bound,
         cs_per_trial = cumsum(saccades_per_trial)) %>%
 # gather(var, value, saccades_per_trial, saccades, cs_per_trial) %>%
  ggplot(aes(occl_bin_bound, saccades_per_trial)) +
  geom_point(alpha = .4) +
  facet_wrap(~participant)
...
occl_ts_saccades %>%
  ...
  mutate(occl_bin = cut_width(occl_ts, .05, boundary = 0)) %>%
  group_by(participant, occl_bin) %>%
  summarise(trials = n_distinct(trial_number),
            occl_bin_bound = min(occl_ts),
            sfreq = sum(saccade_freq),
            mean_freq = mean(saccade_freq)) %>%
  ungroup %>%
  mutate(csf = sfreq/trials) %>%
  ggplot(aes(occl_bin_bound, mean_freq)) + geom_point(alpha = .8) +
  facet_wrap(~participant)
