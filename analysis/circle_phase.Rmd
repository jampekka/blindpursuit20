...
df_trials %>%
  ...
  mutate(block_ts = ts - min(ts)) %>%
  filter(target_class == "target") %>%
  group_by(participant, trial_number) %>%
  slice(1) %>%
  group_by(participant) %>%
  mutate(x = 1:n()) %>%
  ggplot(aes(x, x_rel, shape = factor(success_trial), color = block)) + 
  geom_point(alpha = .9) +
...
tgt <- df_trials %>%
  group_by(participant, block) %>%
  mutate(block_ts = ts - min(ts)) %>%
  filter(target_class == "target", block != 1) %>%
  group_by(participant, trial_number) %>%
  slice(1) %>%
  group_by(participant) %>%
  mutate(x = 1:n()) %>%
...
tgt %>%
  mutate(phase_diff_unwrap = scale(phase_diff_unwrap),
         radius_diff = scale(radius_diff)) %>%
  gather(var, value, phase_diff_unwrap, radius_diff) %>%
  ggplot(aes(x, value, shape = factor(success_trial), color = var)) + 
  geom_point(alpha = .7) +
  facet_grid(participant~.)
  
  
fit <- lmer(radius_diff ~ factor(success_trial) * x + (1|participant), data = tgt)
...
tgt %>%
  mutate(phase_diff_unwrap = scale(phase_diff_unwrap),
         radius_diff = scale(radius_diff)) %>%
  ggplot(aes(x, radius_diff, shape = factor(success_trial), color = factor(success_trial))) + 
  geom_point(alpha = .7) +
  facet_grid(participant~.)
